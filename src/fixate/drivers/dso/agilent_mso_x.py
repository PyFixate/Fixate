...
class MSO_X_3000(DSO):
    ...
    def get_channel_data(self, channel):
        """
        Get all channel data currently displayed on the screen.
        Assumes the data is already captured on the scope, and acquisition is stopped.

        returns: (time_series, data)
        time_series: A list of times in Seconds corresponding to each datapoint.
        data: A list of the data on the screen.

        It is up to the user to know what units the data is in.
        This differs from waveform_values() in that the waveform must already be acquired. This is
        useful when capturing data via a trigger.
        """
        data = []
        time_series = []

        # Check the channel is actually on:
        on = int(self.instrument.query(":CHANnel" + str(channel) + ":DISPlay?"))
        if not on:
            # Channel is off
            raise ValueError("Requested channel is off.")

        # Check if there is actually data to acquire:
        data_available = int(
            self.instrument.query(
                ":WAVeform:SOURce CHANnel" + str(channel) + ";POINTs?"
            )
        )
        if data_available == 0:
            # No data is available
            # Setting a channel to be a waveform source turns it on, so we need to turn it off now:
            self.instrument.write(":CHANnel" + str(channel) + ":DISPlay OFF")
            raise ValueError("No data is available")

        # Now we know the channel is on and has some data in it.
        # Get information about the waveform:
        preamble = self.waveform_preamble()

        acq_type = int(preamble["acquire"])  # 0= NORMAL, 1 = PEAK DETECT, 2 = AVERAGE
        x_inc = float(preamble["x_increment"])  # Time difference between data points.
        x_origin = float(preamble["x_origin"])  # Always the first data point in memory
        x_reference = float(
            preamble["x_reference"]
        )  # Specifies the data point associated with x-origin
        y_inc = float(
            preamble["y_increment"]
        )  # Y INCrement, Voltage difference between data points
        y_origin = float(preamble["y_origin"])  # Y ORIGin, Voltage at center screen
        y_reference = float(
            preamble["y_reference"]
        )  # Y REFerence, Specifies the data point where y-origin occurs, always zero
        ...
        if (
            acq_type == "AVER" or acq_type == "HRES"
        ):  # Don't need to check for both types of mnemonics like this: if ACQ_TYPE == "AVER" or ACQ_TYPE == "AVERage": becasue the scope ALWAYS returns the short form
            points_mode = (
                "NORMal"  # Use for Average and High Resoultion acquisition Types.
            )
        ...
        # Explicitly set this to avoid confusion - only applies to WORD FORMat
        ...
        raw_data = self.instrument.query_binary_values(
            ":WAVeform:SOURce CHANnel" + str(1) + ";DATA?", "h", False
        )
        # Scale it based on the preamble data we got earlier:
        data = [(((i - y_reference) * y_inc) + y_origin) for i in raw_data]

        # Create initial space for time data:
        available_points = int(self.instrument.query_ascii_values(":WAV:POIN?")[0])
        time_series = [i for i in range(0, available_points)]
        # Scale and shift as needed:
        time_series = [(((i - x_reference) * x_inc) + x_origin) for i in time_series]
