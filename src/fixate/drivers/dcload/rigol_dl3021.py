...
class RigolDL3021(DCLoad):
    ...
    def set_current_range(self, current_range: float) -> None:
        ...
        if current_range > 60:
            raise ParameterError(f"Current range {current_range}A exceeds maximum of 60A for DL3021")
        
        if current_range < 0:
            raise ParameterError(f"Current range must be positive. Got {current_range}A")
    ...
    def set_current(self, current: float) -> None:
        ...
        mode = self.get_mode()
        if mode != 'CC':
            raise ParameterError(f"Cannot set current when mode is {mode}. Must be in CONSTANT_CURRENT mode.")
        ...
        if current > self._requested_current_range:
            raise ParameterError(f"Current {current}A exceeds set current range of {self._requested_current_range}A")
