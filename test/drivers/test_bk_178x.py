...
class PacketFormedCorrectly(TestCase):
    ...
    def test_max_voltage_command(self):
        ...
        self.assertTrue(
            self.test_cls.CommandProperlyFormed(
                self._build_byte_array(max_voltage_5_command)
            )
        )
    ...
    def test_checksum(self):
        ...
        max_voltage_5_command[-1] = 0x67
        self.assertEqual(
            self.test_cls.CalculateChecksum(
        ...
        self.assertEqual(
            self.test_cls.CalculateChecksum(self._build_byte_array([0x00] * 25)), 0x00
        )
    ...
    def test_commands(self):
        ...
        test_command[0] = 0xAA
        test_command[-1] = self.test_cls.CalculateChecksum(
            self._build_byte_array(test_command)
        )
        commands = (
            0x20,
            0x21,
    ...
    def test_command_checksum(self):
        ...
        for y in range(0xFF):
            ...
            for x in range(0xFF):
                test_command[-1] = x
                test_resp = x == self.test_cls.CalculateChecksum(
                    self._build_byte_array(test_command)
                )
                self.assertEqual(
                    self.test_cls.CommandProperlyFormed(
...
class BKPS178xInterface(TestCase):
    ...
    def test_little_endian_encode(self):
        ...
        self.assertEqual(
            self.test_cls._little_endian_encode(1 << 8)[:], b"\x00\x01\x00\x00"
        )
        self.assertEqual(self.test_cls._little_endian_encode(1 << 8)[1], 1)
        self.assertEqual(
            self.test_cls._little_endian_encode(1 << 16)[:], b"\x00\x00\x01\x00"
        )
        self.assertEqual(self.test_cls._little_endian_encode(1 << 16)[2], 1)
        self.assertEqual(
            self.test_cls._little_endian_encode(1 << 24)[:], b"\x00\x00\x00\x01"
        )
        self.assertEqual(self.test_cls._little_endian_encode(1 << 24)[3], 1)
        self.assertEqual(
            self.test_cls._little_endian_encode((1 << 32) - 1)[:], b"\xff\xff\xff\xff"
        )

    def test_little_endian_decode(self):
        self.assertEqual(
            self.test_cls._little_endian_decode(
                self.test_cls._little_endian_encode((1 << 32) - 1)
            ),
            (1 << 32) - 1,
        )
        self.assertEqual(
            self.test_cls._little_endian_decode(
                self.test_cls._little_endian_encode(0xAABBCCDD)
            ),
            0xAABBCCDD,
        )
        for x in range(32):
            self.assertEqual(
                self.test_cls._little_endian_decode(
                    self.test_cls._little_endian_encode(1 << x)
                ),
                1 << x,
            )
    ...
    def test_packet_encode(self):
        ...
        command[-1] = sum(command[:-1]) % 256
        self.assertEqual(
            self.test_cls._packet_encode(0x26, (0xFF, 1), (0xAA, 2), (0xBB, 1)), command
        )
    ...
    def test_packet_decode(self):
        ...
        self.assertEqual(
            decoded,
            {"command": 0x26, "address": 0x00, "start": 0xAA, "checksum": command[-1]},
        )
        command[3] = 0xFF
        command[-1] = sum(command[:-1]) % 256
        decoded = self.test_cls._packet_decode(
            self.test_cls._packet_encode(0x26, (0xFF, 1)), ("my0xff", 1)
        )
        self.assertEqual(
            decoded,
            {
        ...
        command[-1] = sum(command[:-1]) % 256
        decoded = self.test_cls._packet_decode(
            self.test_cls._packet_encode(0x26, (0xFF, 1), (0xAABB, 2), (0xCC, 4)),
            ("my0xff", 1),
            ("my0xaabb", 2),
            ("my0xcc", 4),
        )
...
class InstrumentInterface:
    ...
    def StateBinStr(self, binStr):
        ...
        if binStr[7] == "0":
            ...
            out_state = "ON"
        stateStr = [
            "   Operation Mode: " + str(op_mode),
            "   Fan Speed: " + str(fan),
            "   Output Mode: " + str(outp_mode),
            "   Over heat protection: " + str(heat_pro),
